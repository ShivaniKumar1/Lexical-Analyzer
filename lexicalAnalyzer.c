...
int main()
{
  ...
  // Reads through input file until it reaches the end of the file.
	while(!feof(stdin))
	{
		...
		charInputArr[charArr_index] = getchar();
	...
	while(charInputArr[charArr_index] != EOF)
	{
		...
		if(isspace(charInputArr[charArr_index]) != 0)
		{	
			charArr_index++;
			continue;
		}
		...
		else if((isalpha(charInputArr[charArr_index]) != 0) && (isspace(charInputArr[charArr_index]) == 0))
		{
			buff_index = 0;
